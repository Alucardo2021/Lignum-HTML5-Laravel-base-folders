...
/* Swal.fire({
    imageUrl: 'https://assets.afcdn.com/album/D20160818/phalbm24889933.jpg',
    imageHeight: 1500,
...
$('body').on('click', '#boton-x-fecha-editar', function (e){
    ...
    //AGREGAR PELICULA / MODAL / LIMPIAR CAMPOS
/* $('body').on('click', '#agregarPelicula', function (e){
    console.log($('#imagenPeliculaAgregar').prop('files')[0]);
    ...
    $.ajaxSetup({
     ...
     $.ajax({
        ...
        data: {
            "_token": $('meta[name="csrf-token"]').attr('content'),
            'titulo': $('#tituloPeliculaAgregar').val(),
            'año': $('#añoPeliculaAgregar').val(),
            'actor': $('#actorPrincipalPeliculaAgregar').val(),
            'sinopsis': $('#sinopsisPeliculaAgregar').val(),
            'duracion': $('#duracionPeliculaAgregar').val(),
            'imagen': $('#imagenPeliculaAgregar').prop('files')[0],
        },
        processData: false,
        contentType: false,
        dataType: "json",
 ...
 */
 ...
 $('body').on('submit', '#formularioPeliculaAgregar', function (e){
    e.preventDefault();
